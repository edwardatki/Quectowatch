...
static void ble_app_advertise(void) {
  ...
  if (strlen(name) <= sizeof(short_name) - 1) fields.name_is_complete = 1;
  else fields.name_is_complete = 0;
  ...
  if (err) ESP_LOGE(_TAG, "ble_gap_adv_set_fields, err %d", err);
  ...
  if (err) ESP_LOGE(_TAG, "ble_gap_adv_rsp_set_fields fields_ext, name might be too long, err %d", err);
...
static void ble_app_on_sync_cb(void) {
  ...
  if (ret != 0) ESP_LOGE(_TAG, "Error ble_hs_id_infer_auto: %d", ret);
  ble_app_advertise();
}
